...
#[tauri::command]
fn collector_status(app:AppHandle,state:State<CollectorState>)->Result<Value,String>{let running=state.0.lock().ok().and_then(|mut g|g.as_mut().map(|c|c.try_wait().ok().flatten().is_none())).unwrap_or(false);match connection(&app){Ok(db)=>{let checkpoint=db.query_row("SELECT checkpoint_at FROM collector_checkpoint ORDER BY checkpoint_at DESC LIMIT 1",[],|r|r.get::<_,Option<String>>(0)).unwrap_or(None);let has_initial=db.query_row("SELECT count(*) FROM sqlite_master WHERE type='table' AND name='initial_collection_job'",[],|r|r.get::<_,i64>(0)).unwrap_or(0)>0;let job=if has_initial{db.query_row("SELECT job_id,status,(SELECT max(successful_through_month) FROM initial_collection_target WHERE job_id=initial_collection_job.job_id) FROM initial_collection_job ORDER BY updated_at DESC LIMIT 1",[],|r|Ok(json!({"jobId":value_or_empty(r,0),"status":value_or_empty(r,1),"through":value_or_empty(r,2)}))).optional().unwrap_or(None)}else{None};let award_job=if db.query_row("SELECT count(*) FROM sqlite_master WHERE type='table' AND name='award_collection_job'",[],|r|r.get::<_,i64>(0)).unwrap_or(0)>0{db.query_row("SELECT job_id,status,(SELECT max(successful_through_month) FROM award_collection_target WHERE job_id=award_collection_job.job_id) FROM award_collection_job ORDER BY updated_at DESC LIMIT 1",[],|r|Ok(json!({"jobId":value_or_empty(r,0),"status":value_or_empty(r,1),"through":value_or_empty(r,2)}))).optional().unwrap_or(None)}else{None};Ok(json!({"ready":true,"running":running,"checkpoint":checkpoint,"job":job,"awardJob":award_job}))},Err(_)=>Ok(json!({"ready":false,"running":running,"checkpoint":null,"job":null,"awardJob":null}))}}
