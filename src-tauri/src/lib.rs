...
fn search_procurements(app: AppHandle, mode: String, query: Option<String>, category: Option<String>, limit: Option<i64>) -> Result<Value, String> {
    ...
    let sql=match mode.as_str(){
      ...
      _=>"SELECT c.contract_result_id,c.contract_date,c.contract_name,c.demand_institution_name,c.target_detailed_product_class_no,(SELECT t.target_name FROM contract_collection_target t WHERE t.dtil_prdct_clsfc_no=c.target_detailed_product_class_no ORDER BY t.updated_at DESC LIMIT 1),COALESCE((SELECT d.category FROM detailed_product_category d WHERE d.detailed_product_class_no=c.target_detailed_product_class_no AND d.category IS NOT NULL ORDER BY d.updated_at DESC LIMIT 1),'unknown'),NULL,NULL,NULL,c.contract_name,c.contract_amount,0,0,1,'UNLINKED' FROM contract_result c WHERE (?1='' OR c.contract_name LIKE ?2 OR c.demand_institution_name LIKE ?2 OR c.decision_contract_no LIKE ?2 OR c.target_detailed_product_class_no LIKE ?2) AND (?3='all' OR EXISTS(SELECT 1 FROM detailed_product_category d WHERE d.detailed_product_class_no=c.target_detailed_product_class_no AND d.category=?3)) ORDER BY c.contract_date DESC,c.contract_result_id DESC LIMIT ?4"};
...
fn contract_detail(app:AppHandle, contract_result_id:i64)->Result<Value,String>{
    ...
    Ok(json!({"contract":result.0,"raw":{"period":raw_text(&raw,&["cntrctPrd","cntrctBgnDate","cntrctEndDate"]),"businessType":raw_text(&raw,&["bsnsDivNm","cntrctDivNm","cntrctTypeNm"]),"totalAmount":raw_value(&raw,&["totCntrctAmt"]),"privateContractYn":raw_text(&raw,&["prvtCntrctYn","snglContrYn"]),"privateLaw":raw_text(&raw,&["prvtCntrctLawNm","snglContrLawNm"]),"privateBasis":raw_text(&raw,&["prvtCntrctRsn","snglContrRsn","prvtCntrctDtl"]),"delivery":raw_text(&raw,&["dlvrTmlmtDt","dlvrPlce","dlvryCndtnNm","dlvrInfo"]),"businessNo":raw_text(&raw,&["corpBizno","cntrctCorpBizno"]),"company":raw_text(&raw,&["corpNm","cntrctCorpNm"])},"corporations":corp_members(raw_value(&raw,&["corpList"])),"items":items}))
}
