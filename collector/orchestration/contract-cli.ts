...
async function main(){
 ...
 try{
  ...
  while(!cancelled&&(work=nextContractWork(db,id))){
   ...
   if(total>0){for(const {code}of codes)db.prepare("UPDATE contract_month_probe SET status='collecting' WHERE job_id=? AND dtil_prdct_clsfc_no=? AND month=?").run(id,code,work.month);const result=await collectContractRange({database:db,client,discoveryName:work.name,targetCodes:codes.map(({code})=>code),range,isCancelled:()=>cancelled,onProgress:p=>console.log(JSON.stringify({progress:{collector:"contract",month:work!.month,...p}}))});runId=result.runId;if(result.status!=="succeeded")break;}
